...
export default async function RrnaSequencePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [data, mods] = await Promise.all([
    apiFetch<{ units: RrnaUnit[]; rrnaSequence: string }>(`/rrna?species=${id}`),
    apiFetch<ModRow[]>(`/rrna/modifications?species=${id}`),
  ]);
    ...
    <PageShell className="max-w-6xl space-y-4">
      ...
      <section className="rounded-xl border bg-white p-4">
        ...
        <table className="mt-4 w-full text-sm">
          <thead>
            <tr><th className="text-left">Subunit</th><th className="text-left">Start</th><th className="text-left">End</th><th className="text-left">Length</th></tr>
          </thead>
          <tbody>
            {data.units.map((unit) => (
              <tr key={unit.id} className="border-t">
                <td>{unit.subunit}</td><td>{unit.start}</td><td>{unit.end}</td><td>{unit.end - unit.start + 1}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <pre className="max-h-80 overflow-auto rounded bg-slate-100 p-3 text-xs whitespace-pre-wrap break-all">
          {data.units.length
            ? data.units.map((unit, index) => (
                <span key={unit.id} className={unitColors[index % unitColors.length].text}>{unit.sequence ?? ""}</span>
              ))
            : data.rrnaSequence || "No sequence available"}
        </pre>
        <div className="mt-3 flex gap-2">
          <CopyButton text={data.rrnaSequence ?? ""} label="Copy sequence" />
          <a href={`data:text/plain;charset=utf-8,${encodeURIComponent(fasta)}`} download={`${id}-rrna.fasta`} className="rounded border px-3 py-1 text-xs">Download FASTA</a>
        </div>
      </section>
      <RrnaSequenceInteractive organismId={id} units={data.units} modifications={mods} />
    </PageShell>
  );
}
